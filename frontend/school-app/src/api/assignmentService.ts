...
export const assignmentService = {
  ...
  create: (data: AssignmentCreateRequest) => {
    ...
    if (data.latePenaltyPercent !== undefined)
      formData.append('latePenaltyPercent', String(data.latePenaltyPercent));
  ...
  update: (id: number, data: AssignmentUpdateRequest) => {
    ...
    if (data.anonymousGrading !== undefined)
      formData.append('anonymousGrading', String(data.anonymousGrading));
    return apiClient.put<AssignmentResponse>(`${BASE}/${id}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
  },
  ...
  // NEW: Publish assignment (teacher/admin)
  publish: (id: number) =>
    apiClient.patch<AssignmentResponse>(`${BASE}/${id}/publish`),

  // Submissions
  getSubmissionsForAssignment: (assignmentId: number, page = 0, size = 20) =>
    apiClient.get<PageResponse<SubmissionResponse>>(`${BASE}/${assignmentId}/submissions`, {
      params: { page, size },
    }),
  ...
  submit: (assignmentId: number, data: SubmissionRequest) => {
    ...
    return apiClient.post<SubmissionResponse>(`${BASE}/${assignmentId}/submit`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
  },

  grade: (submissionId: number, data: GradeSubmissionRequest) =>
    apiClient.put<SubmissionResponse>(`${BASE}/submissions/${submissionId}/grade`, data),

  getSubmissionsByStudent: (studentId: number, page = 0, size = 20) =>
    apiClient.get<PageResponse<SubmissionResponse>>(`${BASE}/submissions/student/${studentId}`, {
      params: { page, size },
    }),
